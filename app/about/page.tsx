  ...
  return (
    <section className={`pt-32 ${noto.className} bg-[#f0f1eb]`}>
      ...
      <main className={ibm.className}>
        ...
        {/* profile details */}
        <section className="w-[55vw] mx-auto flex gap-6 mb-40">
          ...
          {/* stacks */}
          <div className="bg-white py-12 px-6 shadow-sm rounded-2xl w-[35vw]">
            <header>
              <div className="text-4xl font-semibold mb-5">My Tech Stacks</div>
              <div className="text-2xl text-[#404040]">
                새로운 스택에도 열려있습니다
              </div>
            </header>
            {/* stack icons */}
        ...
        {/* contact icons */}
        {/* <div className="w-[60vw] mx-auto flex items-center justify-end gap-6 relative">
          <ContactIcon
            icon={<IoIosMail size={"65%"} />}
            path="mailto:hanganda23@naver.com"
          />
        ...
        {/* texts */}
        <section className="bg-white py-72 rounded-t-[50%]">
          <header>
...
const ProfileField: FC<ProfileFieldProps> = ({ content, label }) => {
  return (
    <div className="flex items-center gap-5">
      <label className="text-xl w-28">{label}</label>
      <div className="text-lg bg-white shadow-sm rounded-lg px-3 py-2 flex-1">
        {content}
      </div>
    </div>
  );
...
const ContactIcon: FC<ContactIconProps> = ({ icon, path }) => {
  return (
    <Link href={path} target="_blank">
      <div className="w-24 h-24 overflow-hidden rounded-2xl shadow-sm bg-white flex items-center justify-center text-gray-600">
        {icon}
      </div>
    </Link>
  );
