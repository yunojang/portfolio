    ...
    <section className="bg-white py-80 text-container">
      ...
      <div className="flex flex-col gap-32 w-[45vw] mx-auto ">
        <CoverLetter
          ...
          content="진로를 선택하는 과정에서 IT 전문학교에 입학 해보았고, 2년간 정보보호를 전공으로 네트워크, 암호학, 포렌식 등 여러 분야를 학습했습니다. 다양한 영역에 관심이 있는게 특징인데, 학습하는 과정에도 관심가는 분야들을 따로 공부 하기도 했고 당시, JAVA나 C언어를 통해 개발하는 것에 가장 흥미를 가져 졸업한 후에, 개발 집중 학습을 위한 강좌와 교육을 수료했습니다. JAVA Spring으로 웹 백엔드를 개발하는 교육이었으나, 웹개발 중 JS로하는 프론트 개발에 더 흥미를 느껴 수료 이후 프론트 개발자로 취업했습니다. 2개의 회사에 총 3년 재직(3개월 + 2년 9개월)하면서 주로, Next, React로 웹 프론트를 개발하며 요구에 맞는 개발과 함께, CRA같은 사내 보일러 플레이트를 제작하며 빌드 과정과 구조를 이해하며, 사내 컴포넌트 패키지 개발 경험도 있습니다. 웹 반응형 개발 및 React-native와 Webview를 사용하여 2개의 동일한 소스를 기반으로 운영되는 웹과 앱을 개발하기도 하였습니다."
        />

        <CoverLetter
          title="함께 일 하는 스타일"
          content="기본적으로 내향적인 유형이지만, 사람에게 먼저 다가가고 말도 많이 하는 편입니다. 밝고 두루두루 잘 지내는 편이라 타 부서 사람들과도 원만히 지내 부서 간 소통 창구 역할을 하기도 했습니다. 업무에서는 주로 프론트 리드를 맡아 프로젝트 별 구조 및 사용 스택을 결정하여 진행했습니다. 새 프로젝트를 개발에서 서버와 프론트가 다른 일정으로 따로 개발하는게 우려되어, 서버 팀 마다 프론트 개발 진행사항과 그에 맞는 API 요구사항을 작성하여 공유 및 조율 회의를 주관하여, 무사히 일을 마칠 수 있었습니다. 비 효율적 회의에 힘듦을 느껴 주관하는 회의에는 확실한 목적을 세우고 철저한 준비 후에 의견 교류에만 집중된 회의가 이루어지도록 노력합니다. 사담도 좋아하지만, 회의시간 보다 별도의 시간을 가지는 것이 좋았습니다. 회사에 불만이 생길 수 있는데, 막연한 불만으로 담아두기 보다 구체적으로 정리해야 개선의 여지가 있다고 믿기에, 기록 및 정리 해 두었다가 혼자서도 개선할 수 있는 부분을 찾아 노력한 뒤에도 안된다면 기회가 될 때, 상급자(관리자)에게 말하는 편입니다. 그렇게 정리된 내용 및 노력을 전해 함께 많은 부분을 개선할 수 있었습니다."
        />
